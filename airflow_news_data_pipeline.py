...
with dag:
	...
	bash_command='aws s3 mv {{ ti.xcom_pull("extract_news_info")}}  s3://newscovid-vishnu',
	)
	
	snowflake_create_table=SnowflakeOperator(
		task_id="snowflake_create_table",
		sql="""create  table if not exists helloparquet using template(select ARRAY_AGG(OBJECT_CONSTRUCT(*)) from TABLE(INFER_SCHEMA (LOCATION=>'@news_covid_db.news_schema.snow_simple',FILE_FORMAT=>'parquet_format')))
        """ ,
	...
	snowflake_copy=SnowflakeOperator(
		...
		sql="""copy into news_covid_db.news_schema.helloparquet from @news_covid_db.news_schema.snow_simple MATCH_BY_COLUMN_NAME=CASE_INSENSITIVE FILE_FORMAT=parquet_format
        """ ,
		snowflake_conn_id="snowflake_conn"
	)
